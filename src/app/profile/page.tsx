  ...
  return (
    <main className="min-h-screen bg-paper pb-10">
      <div className="flex items-center justify-between px-5 py-4 border-b border-line">
        ...
        <button onClick={logout} className="text-xs text-fg/50 hover:text-blue transition-colors">
          Log out
        </button>
      </div>
      ...
      <div className="max-w-2xl mx-auto px-5 py-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-1.5">
              <h1 className="font-display text-xl font-bold text-fg">{user.full_name}</h1>
              {user.is_verified && (
                <span className="text-blue text-sm" title="Verified">✓</span>
              )}
            </div>
            <div className="text-sm text-fg/50">@{user.username}</div>
            <div className="text-xs text-fg/40 capitalize mt-0.5">{user.role}</div>
          </div>
        </div>
        ...
        <div className="mt-6 space-y-3">
          ...
          {user.is_verified && user.is_premium && (
            <p className="text-sm text-fg/40 text-center py-4">
